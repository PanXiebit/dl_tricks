...
def read_cifar10(data_file, batch_size):
    ...
    data_files = tf.gfile.Glob(data_file)  # <class 'list'>: ['dataset/data_batch_1']
    ...
    key, value = reader.read(file_queue) # 表示从文件中每次读取这个多字节的信息吧?
    ...
    # 将长度为[depth*height*width] 的字符串转换为形如 [depth, height, width] 的图片张量, 数据格式的组织形式就是这样的
    ...
    example_queue = tf.RandomShuffleQueue(   # tf.train.shuffle_batch 有啥区别
        capacity= 8 * batch_size + 4 * batch_size,            # 样例队列的容量，通常设置为 min_affter_dequeue + (num_threads + margin)*batch_size
    ...
    # class QueueRunner(object): Holds a list of enq ueue operations for a queue, each to be run in a thread.
...
if __name__ == "__main__":
    with tf.Session() as sess:
        ...
        batch_xs, batch_ys = read_cifar10("dataset/data_batch_1", batch_size=1)
